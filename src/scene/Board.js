...
export class Board {
  ...
  buildSpaces() {
    for (let i = 1; i <= 30; i++) {
      ...
      // Tile base
      const tileSize = SPACE_SIZE;
      const tileGeo = new THREE.BoxGeometry(
        tileSize + wobble(0.04),
        0.12,
        tileSize + wobble(0.04)
      );
      ...
      let tileColor;
      if (space.bonus) {
        tileColor = 0x6bcb77; // green for bonus
      } else if (space.type === '+') {
        tileColor = 0xfff8e7; // warm white for plus
      } else {
        tileColor = 0xf0f0ff; // cool white for minus
      }
      ...
      if (!space.bonus) {
        ...
        // Bonus label — show '?' since direction is random, or 'WIN' for space 30
        const bonusLabel = space.num === 30 ? 'WIN' : '?';
